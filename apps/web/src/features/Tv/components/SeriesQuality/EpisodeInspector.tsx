...
export const EpisodeInspector = ({ selection, tvId }: Props) => {
  if (selection === null) {
    return (
      <aside
        className="flex min-h-72 flex-col justify-center border-t border-border bg-surface-raised/50 p-6 min-[860px]:border-t-0 min-[860px]:border-l min-[860px]:p-8"
        aria-live="polite"
      >
        <p className="text-micro font-medium tracking-[0.18em] text-muted-foreground uppercase">
          Episode inspector
        </p>
        <h2 className="mt-3 text-xl font-light">Select a rated episode</h2>
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 inline-flex items-center gap-2 text-5xl font-extralight text-primary tabular-nums">
        <Star aria-hidden="true" className="size-6 fill-current" />
        {selection.voteAverage.toFixed(1)}
      </p>
      <h2 className="mt-5 text-lg font-normal">{selection.episode.name}</h2>
      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
